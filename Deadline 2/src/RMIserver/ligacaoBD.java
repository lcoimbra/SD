...
public class ligacaoBD {
    ...
    private static final String apiKey = pbd.getProperty("apiKey");
    private static final String apiSecret = pbd.getProperty("apiSecret");
    ...
    synchronized public String createAuction(int utilizador, long codArtigo, String title, String description, String deadline, double amount) throws SQLException {
       ...
       Connection ligacao = null;
        ...
        try {
            ...
            ArrayList<String> fbInfo = fbGetInfo(utilizador);

            if (fbInfo != null && fbInfo.get(0) != null) {

                //search.setCode(this.Code);
                String resSearch = searchAuction(codArtigo);
                TradutorComandos tc = new TradutorComandos(resSearch);
                int itemCount = Integer.parseInt(tc.getM().get("items_count").toString());
                int idAuction = Integer.parseInt(tc.getM().get("items_" + (itemCount - 1) + "_id").toString());
                ...
                Token accessToken = new Token(fbInfo.get(1), "");
                OAuthRequest request = new OAuthRequest(Verb.POST, "https://graph.facebook.com/" + fbInfo.get(0) + "/feed", service);
                request.addBodyParameter("message", "I just created a new auction. Come check it out!");
                request.addBodyParameter("link", "http://eden.dei.uc.pt/~amaf/echo2.php?redirect=http://localhost:8080/DetailAuction?Id=" + idAuction);
    ...
    synchronized public String detailAuction(int auctionID) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                res = "type: detail_auction " + ", code: " + rs.getLong("artigo") + ", title: " + rs.getString("titulo") + ", description: " + rs.getString("descricao") + ", amount: " + rs.getDouble("preco_max") + ", deadline: " + rs.getTimestamp("data_fim").toString().replaceAll(":", "-").substring(0, 16) + ", messages_count: " + i + ", ";
    ...
    synchronized public String editAuction (int utilizador, int auctionID, long codArtigo, String title, String description, String deadline, double amount) throws SQLException {
        ...
        String selectSQL = "SELECT leilaoID FROM leilao WHERE leilaoID = ? AND utilizadorID = ?";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (deadline != null) {
                    if (previous)
                        selectSQL2 = selectSQL2 + ",";
                    selectSQL2 = selectSQL2 + " data_fim = TO_TIMESTAMP(?, 'RR-MM-DD HH24:MI:SS')";
                    previous = true;
                }
                if (amount != -1) {
                    if (previous)
    ...
    synchronized public String stats (int utilizador) throws SQLException {
        ...
        try {
            ...
            String selectSQL3 = "SELECT NVL(vencedor,0) as vence, COUNT(leilaoID) as contagem FROM leilao GROUP BY vencedor ORDER BY contagem DESC";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (utilizadorWin == 0) {
                    ...
                    selectSQL2 = "UPDATE leilao SET estado = 1, vencedor = ? WHERE leilaoID = ?";
                    preparedStatement2 = ligacao.prepareStatement(selectSQL2);
    ...
    synchronized public int fbVerifyUser(long fb_id, String fb_token) throws SQLException {
        Connection ligacao = null;
        PreparedStatement preparedStatement = null, preparedStatement2 = null, preparedStatement3 = null, preparedStatement4 = null;
        ...
        try {
            ...
            String selectSQL = "SELECT utilizadorID FROM utilizador WHERE fb_id = ?";

            preparedStatement = ligacao.prepareStatement(selectSQL);
            preparedStatement.setLong(1, fb_id);

            /* execute select SQL stetement*/
            ResultSet rs = preparedStatement.executeQuery();

            if (rs.next()) {
                String selectSQL2 = "UPDATE utilizador SET fb_token = ? WHERE fb_id = ?";

                preparedStatement2 = ligacao.prepareStatement(selectSQL2);
                preparedStatement2.setString(1, fb_token);
                preparedStatement2.setLong(2, fb_id);
...
/*                String selectSQL3 = "INSERT INTO utilizador (utilizadorid, previlegios, banido, fb_id, fb_token) VALUES (s_utilizador.nextVal,0, 0, ?, ?)";

                preparedStatement3 = ligacao.prepareStatement(selectSQL3);
                preparedStatement3.setLong(1, fb_id);
                preparedStatement3.setString(2, fb_token);


                preparedStatement3.executeQuery();

                String selectSQL4 = "SELECT utilizadorID FROM utilizador WHERE fb_id = ?";

                preparedStatement4 = ligacao.prepareStatement(selectSQL4);
                preparedStatement4.setLong(1, fb_id);


                ResultSet rs4 = preparedStatement4.executeQuery();
                rs4.next();

                return rs4.getInt("utilizadorID");
*/
                return -1;
            }
        } catch (SQLException e) {
    ...
    synchronized public boolean fbAssociate(int userID, long fb_id, String fb_token) throws SQLException {
        Connection ligacao = null;
        PreparedStatement preparedStatement = null, preparedStatement2 = null;

        try {

            if (fbIsAssociated(userID))
                return false;   // ja associado
            ...
            if (rs.next()) {
                ...
                String selectSQL2 = "UPDATE utilizador SET fb_id = ?, fb_token = ? WHERE utilizadorID = ?";

                preparedStatement2 = ligacao.prepareStatement(selectSQL2);
                preparedStatement2.setLong(1, fb_id);
                preparedStatement2.setString(2, fb_token);
    ...
    synchronized public boolean fbIsAssociated(int userID) throws SQLException {
        Connection ligacao = null;
        PreparedStatement preparedStatement = null;
        ...
        try {
            ...
            String selectSQL = "SELECT NVL(fb_id, 0) as fb_id FROM utilizador WHERE utilizadorID = ?";
    ...
    synchronized public ArrayList<String> fbGetInfo(int userID) throws SQLException {
        Connection ligacao = null;
        PreparedStatement preparedStatement = null;
        ...
        try {
            ...
            String selectSQL = "SELECT fb_id, fb_token as fb_token FROM utilizador WHERE utilizadorID = ?";
            ...
            if (rs.next()) {
                array.add(rs.getString("fb_id"));
                array.add(rs.getString("fb_token"));    // null OR token
                return array;
            }
        } catch (SQLException e) {
            //e.printStackTrace();
    ...
    synchronized public boolean fbUnlinkAccount(String user) throws SQLException {
        Connection ligacao = null;
        PreparedStatement preparedStatement = null, preparedStatement2 = null;
        ...
        try {
            ...
            if (rs.next()) {
                String selectSQL2 = "UPDATE utilizador SET fb_id = NULL, fb_token = NULL WHERE utilizador = ?";
    ...
    synchronized public String getUsername(int userID) throws SQLException {
        Connection ligacao = null;
        PreparedStatement preparedStatement = null;
        ...
        try {
            ...
            String selectSQL = "SELECT utilizador FROM utilizador WHERE utilizadorID= ?";
            ...
            if (rs.next()) {
                return rs.getString("utilizador");
            }
        } catch (SQLException e) {
            //e.printStackTrace();
            //System.out.println(e.getMessage());
